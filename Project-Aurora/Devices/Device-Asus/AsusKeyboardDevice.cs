...
{
    class AsusKeyboardDeviceWrapper : AsusGenericDeviceWrapper
    {
        protected override AuroraDeviceType AuroraDeviceType => AuroraDeviceType.Keyboard;
        public AsusKeyboardDeviceWrapper(IAuraSyncDevice keyboard, AsusDeviceConnector connector) : base(keyboard, connector) { }

        protected override void AsusUpdateDeviceKey(KeyValuePair<DeviceKeys, Color> key)
        {
            ushort keyCode = (ushort)LedMap[key.Key];
            SetRGBColor(GetAuraRgbLight(keyCode), key.Value);
        }

        public override void Initialize()
        {

            foreach (IAuraRgbKey key in ((IAuraSyncKeyboard)Device).Keys)
            {
                LedMap[AsusLedIdMapper(key.Code)] = key.Code;
            }
            //UK Layout
            ushort asusCode = 0x56;
            LedMap[AsusLedIdMapper(asusCode)] = asusCode;
            asusCode = 0x59;
            LedMap[AsusLedIdMapper(asusCode)] = asusCode;
        }

        private IAuraRgbLight GetAuraRgbLight(ushort keyCode)
        {
