...
namespace Device_LightFX
{
    public class LightFxConnector : AuroraDeviceConnector
    {
        protected override string ConnectorName => "LightFx";

        protected override bool InitializeImpl()
        {
            devices.Add(new LightFxDevice());
            return true;
        }
    ...
    class LightFxDevice : AuroraDevice
    {
        protected override string DeviceName => "LightFX";
        ...
        public int deviceStatus()
        {
            if (usingHID) {
                ...
                if (!read) {
                    LogInfo("Error Code: " + Marshal.GetLastWin32Error());
                    return -1;
                }
        ...
        protected override bool UpdateDeviceImpl(DeviceColorComposition composition)
        {
            ...
            {
                LogError("LightFX device, error when updating device. Error: " + exc);
        ...
        {
            local.Register($"{DeviceName}_custom_pid", false, "Use Custom PID");
            local.Register($"{DeviceName}_pid", 0, "Device PID: 0x", flags: VariableFlags.UseHEX);
            local.Register($"{DeviceName}_length", true, "Use 12 byte data");
        }
    }
}